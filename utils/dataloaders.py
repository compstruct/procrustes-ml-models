...
def get_dali_train_loader(dali_cpu=False):
    def gdtl(data_path, batch_size, workers=5, input_size=224, _worker_init_fn=None):
        if torch.distributed.is_initialized():
            local_rank = torch.distributed.get_rank()
            world_size = torch.distributed.get_world_size()
        else:
...
def get_dali_val_loader():
    def gdvl(data_path, batch_size, workers=5, input_size=224, _worker_init_fn=None):
        if torch.distributed.is_initialized():
            local_rank = torch.distributed.get_rank()
            world_size = torch.distributed.get_world_size()
        else:
