...
def main():
    global args, best_prec1, cp_logger
    ...
    if args.drop_back == 'True':
        ...
        optimizer = Dropback(model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay, track_size=args.track_weights, init_decay=args.init_decay, named_params=model.named_parameters())
    else:
        print("Baseline Optimizer is used!")
        optimizer = torch.optim.SGD(model.parameters(), args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.2, patience=5, verbose=False, threshold=1e-1, threshold_mode='abs', cooldown=0, min_lr=0, eps=1e-8)
...
def train(train_loader, train_loader_len, model, criterion, optimizer, epoch):
    ...
    #---------------------------------------------------------------------------------------
    #EXTENDED
    # This will be reinitialized at each epoch.
    hooks =[]
    # modle.module as it is wrapped in DataPArallel now ! 
    for name, module in model.module.named_modules():
        if type(module)==nn.Conv2d or  type(module)==nn.Linear:
            hook_name=str(name)+'_'+str(module).split('(', 1)[0]
            print("=======================================================================================================")
            print('Attaching hook to %s - %s' % (hook_name, str(module)) )
            hooks.append(Hook(module, hook_name))
        #else:
        #    print('leaving out %s - %s' % (hook_name, str(module)))

    epoch_start_dumping = 0 # change it to 4 later
    hooks_enables = False;
    #---------------------------------------------------------------------------------------
    ...
    #   ---------------------------------------------------------------------------------------
    #EXTENDED
        #every 5 epochs # only dump the first itteration
        if (epoch >= epoch_start_dumping) & (i == 0):
            for h in hooks:
                h.enable()
            hooks_enables = True
            print("Hooks are enabled at epoch #%d" % epoch)
            if args.drop_back == 'True':
                optimizer.enable_dumping()
                print("Weight dump enabled at epoch #%d" % epoch)
        else:
            for h in hooks:
                h.disable()
            hooks_enables = False
            if args.drop_back == 'True':
                optimizer.disable_dumping()
    #---------------------------------------------------------------------------------------

        if args.drop_back == 'True':
            dump_path = 'dumps/'+str(args.arch)+'_dp-'+str(args.drop_back)+'_wt-'+str(args.track_weights) + '_ep-'+str(epoch)+'_itt-'+str(i)
            optimizer.dump_init(dump_path)
